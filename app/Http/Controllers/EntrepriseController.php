...
class EntrepriseController extends Controller
{
    ...
    public function get_entreprise_byid($id)
    {
        ...
        if (is_null($entreprise)) {
            ...
            return response()->json(['data' => 'entreprise not found'], 404);
        }
    ...
    public function update_entreprise(Request $request, $id)
    {
        ...
        //**********************************
        $tableupdate = [];
        if (!empty($request->raison_sociale)) {$tableupdate['raison_sociale'] = $request->raison_sociale;}
        if (!empty($request->activite)) {$tableupdate['activite'] = $request->activite;}
        if(!empty($request->forme_juridique)){ $tableupdate['forme_juridique'] = $request->forme_juridique;}
        if(!empty($request->responsable)){ $tableupdate['responsable'] = $request->responsable;}
        if(!empty($request->mobile)){ $tableupdate['mobile'] = $request->mobile;}
        if(!empty($request->email_res)){ $tableupdate['email_res'] = $request->email_res;}
        if(!empty($request->siege)){ $tableupdate['siege'] = $request->siege;}
        if(!empty($request->rib)){ $tableupdate['rib'] = $request->rib;}
        if(!empty($request->prevision)){ $tableupdate['prevision'] = $request->prevision;}
        DB::table('entreprise')
            ->where('user_id', $id)
            ->update($tableupdate);
