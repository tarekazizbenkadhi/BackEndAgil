...
class CommandeBonValeurController extends Controller
{
    ...
    public function get_commande_client_bv_byCin($cin)
    {
    ...
    public function get_commande_client_bv_bydate($date_rv)
    {
        ...
        $bv = DB::table('commande_bon_valeurs as c')
            ...
            ->where('c.etat', '=', '0')
            ->where('client.cin', '!=', 'null')
            ->where('rendez_vous_bvs.date_time_rv', $date_rv)
            ->select('users.*', 'client.*', 'c.*', 'rendez_vous_bvs.*')
            ->get();
        if (is_null($bv)) {
    ...
    public function get_commande_entreprise_bv_byMat($mat)
    {
        $bv = DB::table('commande_bon_valeurs as c')
            ->leftJoin('users', 'users.id', '=', 'c.user_id')
            ->leftJoin('entreprise', 'entreprise.user_id', '=', 'users.id')
    ...
    public function get_commande_entreprise_bv_bydate($date_rv)
    {
        $bv = DB::table('commande_bon_valeurs as c')
            ->leftJoin('users', 'users.id', '=', 'c.user_id')
            ->leftJoin('entreprise', 'entreprise.user_id', '=', 'users.id')
